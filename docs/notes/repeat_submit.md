# 🔄 Spring AOP + Redis 解决重复提交

> 💡 防止接口重复提交 | AOP 切面 | Redis 分布式锁

---

## 📖 问题描述

用户在点击操作的时候，可能会连续点击多次。虽然前端可以通过设置按钮的 `disabled` 属性来控制按钮不可连续点击，但是如果别人拿到请求进行模拟，依然会出现问题。

项目是用 JWT 进行认证的，所以用的 `token + url` 来作为 key，value 无所谓，因为用不到 value。
