...
> 💡 通过导出导入方式快速创建多个 WSL 实例，并配置固定 IP 实现互通
...
# 或安装完整 Hyper-V（推荐，安装成功后再执行命令就不会报错了）
...
> 2. 执行 `sudo netplan apply` 可能会报如下警告：
>
> ```text
...
> ```
>
> 这两个警告不影响功能，可以忽略。但第二个警告说明 `systemd-networkd` 可能没运行，导致静态 IP 没生效。执行 `wsl -d ubuntu-101 hostname -I` 查看 IP 通常是生效的。
>
> `Open vSwitch` 是可选的虚拟交换机工具，WSL 不需要它。
...
因为三台机器都是通过同一个 backup tar 导入的，按照上述方式设置 IP 后，可能出现三台机器的 IP 都是 `192.168.3.101` 的情况。
...
未来如果要添加 `104`、`105`、`106` 等节点，仍然可以使用以下方式解决：
